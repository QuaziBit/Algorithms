...
public class Main
{
    public static void main(String[] args)
    {
        ...
        */
        ...
        // Lets use randomly generated array to test Binary Search Algorithm
        numbers = generateRandomArray(range);

        // Getting some number from the middle of the random array,
        // and will search for that number
        searchNum = numbers[ (range / 2) - 1 ];
    ...
    private static int[] generateRandomArray(int n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            int ran = (int) Math.round( (Math.random() * n) );
            random_array[i] = ran;
        }
