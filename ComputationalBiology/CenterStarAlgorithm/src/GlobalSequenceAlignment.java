...
public class GlobalSequenceAlignment
{
        ...
        for (int i = 1; i < V.length; i++)
        {
            ...
            for (int j = 1; j < V[i].length; j++)
            {
                ...
                else
                {
                    ...
                    else if (insrDellScore >= 0)
                    {
                        // *** IMPORTANT NOTE *** //
                        // ------------------------------------------------------------------ //
                        // Fixing bug when for some reason we cannot insert mismatch value.
                        // This fix mostly needed for the local string alignment,
                        // and not for the global string alignment.

                        // if you need work with the global string alignment
                        // you have to remove this 'else-if statement' because for
                        // the global string alignment you always have to have
                        // negative values for gap-penalty and for mismatch.
                        // ------------------------------------------------------------------ //
                        V[i][j] = misVal;
                    }
     ...
     * This method will trace-back the scoring-matrix,
    ...
    private void stringAlignment(int i, int j)
    {
        ...
        else
        {
            ...
            if (i > 0 && j == 0)
            {
                horizontalVal = V[i][j] + insrDellScore;
                verticalVal = V[i - 1][j] + insrDellScore;
                misVal = V[i - 1][j] + missScore;
            }
            else if (i == 0 && j > 0)
            {
                horizontalVal = V[i][j - 1] + insrDellScore;
                verticalVal = V[i][j] + insrDellScore;
                misVal = V[i][j - 1] + missScore;
            }
            */
            ...
            else if (misVal == horizontalVal && misVal == verticalVal && horizontalVal == verticalVal)
            {
                ...
                // so it is really doesn't matter which way we go.
            ...
            else
            {
                // *** IMPORTANT NOTE *** //
                // ------------------------------------------------------------------ //
                // Fixing bug when for some reason we cannot cannot back-trace
                // due to positive value of gap-penalty.
                // This fix mostly needed for the local string alignment,
                // and not for the global string alignment.

                // if you need to work with the global string alignment
                // you have to remove this 'else statement' because for
                // the global string alignment you always have to have
                // negative values for gap-penalty and for mismatch.
                // ------------------------------------------------------------------ //

                // If our 'insrDellScore' equals to 0 or more than 0
